...
async function loadAll() {
  ...
  try {
    ...
    renderPage();
    showNotif(`✓ ${state.products.length} produk, ${state.driveImages.length} foto dimuat`);
  } catch(e) {
    showNotif('Gagal memuat data: ' + e.message);
  }
}
...
async function runAIScoring() {
  ...
  if (!f.productName?.trim()) { showNotif('Masukkan nama produk terlebih dahulu'); return; }
  ...
  try {
    ...
    state.isAnalyzing = false;
    showNotif('Gagal analisis: ' + e.message);
    renderPage();
  }
}
...
async function saveCurrentProduct() {
  ...
  try {
    ...
    state.scoringResult = null;
    state.scoringAiData = null;
    showNotif(`"${product.name}" tersimpan ke Sheets ✓`);
    navigate('products');
  } catch(e) {
    showNotif('Gagal menyimpan: ' + e.message);
  }
}
...
function confirmDelete(id, name) {
  showModal(`Hapus produk "<strong>${escHtml(name)}</strong>"?<br>Data di Sheets akan ikut terhapus.`, async () => {
    try {
      await API.deleteProduct(id);
      state.products = state.products.filter(p=>String(p.id)!==String(id));
      renderPage();
      showNotif('Produk dihapus dari Sheets ✓');
    } catch(e) {
      showNotif('Gagal hapus: ' + e.message);
    }
  });
}
...
async function handleImgUpload(e) {
  ...
  showNotif(`☁️ Mengupload ${files.length} foto ke Drive...`);
  try {
    for (const file of files) {
      const b64 = await fileToBase64(file);
      const res = await API.uploadImage(file.name, b64, file.type);
      if (res.imageUrl) {
        state.driveImages.push({ name: file.name, imageUrl: res.imageUrl, fileId: res.fileId });
      }
    }
    renderPage();
    showNotif(`✓ ${files.length} foto tersimpan di Google Drive`);
  } catch(e) {
    showNotif('Gagal upload: ' + e.message);
  }
  e.target.value = '';
}
...
function showAssignImg(productId, productName) {
  if (state.driveImages.length === 0) {
    showNotif('Upload foto terlebih dahulu di bagian atas!');
    return;
  }
  const items = state.driveImages.map((img, i) => `
    <div class="selectable-item" onclick="doAssignImg('${productId}','${img.imageUrl}',this)" style="margin-bottom:6px;cursor:pointer">
  ...
  showModal(`<div style="font-size:14px;font-weight:700;color:var(--wood);margin-bottom:12px">Pilih foto untuk:<br>"${escHtml(productName)}"</div><div style="max-height:280px;overflow-y:auto">${items}</div>`, null, true);
}

async function doAssignImg(productId, imageUrl, el) {
  closeModal();
  showNotif('🔗 Assign foto ke produk...');
  try {
    ...
    renderPage();
    showNotif('Foto berhasil di-assign ✓');
  } catch(e) {
    showNotif('Gagal assign: ' + e.message);
  }
}
...
async function saveBundleToSheets() {
  const b = state.bundleResult;
  if (!b) return;
  const copy = generateCopy(b, null, null);
  const data = {
    productIds: b.products.map(p=>p.id),
    productNames: b.products.map(p=>p.name).join(', '),
    totalPrice: b.totalPrice,
    bundlePrice: b.bundlePrice,
    discountPct: b.discountPct,
    savings: b.savings,
    overlappingStages: b.overlappingStages,
    skills: b.skills,
  ...
  try {
    await API.saveBundle(data);
    showNotif('Bundle tersimpan ke Sheets ✓');
  } catch(e) {
    showNotif('Gagal simpan bundle: ' + e.message);
  }
}
...
async function saveGasUrl() {
  ...
  showNotif('🔄 Testing koneksi...');
  const ok = await API.checkConnection();
  if (ok) {
    showNotif('✅ Koneksi berhasil! Memuat data...');
    await loadAll();
  } else {
    showNotif('❌ Gagal konek. Cek URL dan pastikan GAS sudah di-deploy.');
  }
}
...
function showModal(html, onConfirm, noButtons = false) {
  ...
  box.innerHTML = `
    <div style="padding:24px">
      <div style="font-size:14px;color:var(--text2);line-height:1.6;margin-bottom:${noButtons?0:20}px">${html}</div>
      ${!noButtons?`<div class="flex gap-8 justify-end">
        <button class="btn btn-outline btn-sm" onclick="closeModal()">Batal</button>
        <button class="btn btn-primary btn-sm" onclick="modalConfirm()">Ya, Hapus</button>
      </div>`:''}
    </div>`;
...
function afterRender() {
  if (state.currentPage === 'dashboard') {
    ...
    state.chart = new Chart(ctx,{
      type:'bar',
      data:{labels:STAGES.map(s=>s.id),datasets:[{label:'Produk',data:STAGES.map(s=>Math.round((dist[s.id]||0)*10)/10),backgroundColor:'rgba(139,99,67,0.7)',borderColor:'#8B6343',borderWidth:1.5,borderRadius:6}]},
      options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true,grid:{color:'rgba(139,99,67,0.1)'},ticks:{color:'#9C7B63',font:{size:11}}},x:{grid:{display:false},ticks:{color:'#6B5043',font:{size:12,weight:'600'}}}}}
    });
  }
}
...
// ── INIT ──────────────────────────────────────────────────────
(async function init() {
  // Load saved GAS URL from localStorage
